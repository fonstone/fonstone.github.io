...
### L2：多会话检索（Multi-session Retrieval）
...
### 长期记忆（Long-term Memory）：跨会话的持久知识
...
*图 S-2 四种存储格式的取舍关系与选择策略*

## 用户记忆的进阶表示：User as Code
...
*图 S-3 Mem0 记忆管理管道的三个阶段*

![](/images/courses/ai-agent/fig3-4.svg)

*图 S-4 Memobase 的双层记忆架构与缓冲批处理*
...
*图 S-7 Re-ranking 的 Two-stage 检索架构*
...
*图 S-8 至 S-15 RAG 技巧全景图谱：查询重写、滑动窗口、分层摘要与多轮检索等技术*

## 总结

用户记忆系统不是简单的"信息存储工具"，而是一个**构建用户预测模型**的认知系统。从选择性记录到结构化存储，从文本记忆到可执行代码，从平面检索到知识图谱推理，这条演进路径揭示了一个核心趋势：**智能体的记忆能力正在从"被动存储"走向"主动理解"**。

对于一个完整的智能体记忆系统，我们需要同时关注三个维度：**记忆什么**（选择性）、**如何组织**（结构性）和**如何利用**（检索与推理）。这三个维度的设计质量，最终决定了智能体能否从"记住了"真正进化到"理解了"。

在 RAG 方面，从基础的向量检索到知识图谱 RAG，再到智能体化的 Self-RAG、Corrective RAG 和 Adaptive RAG，我们看到检索技术正在变得更加智能化、自适应。而与技巧层面的改进（上下文感知检索、HyDE、Re-ranking）相结合，RAG 正在从一个简单的检索管道，演进为一个完整的智能知识管理系统。

最后，回到本篇开篇的问题：**智能体应该记住什么？** 答案不是"记住所有信息"，也不是"记住关键信息"，而是**记住那些能够帮助构建更好的用户预测模型的信息**。从这个角度来看，记忆系统的设计不仅仅是一个存储问题，更是一个认知建模问题——而这正是用户记忆系统区别于传统数据库的根本之处。

当你的智能体能够通过它对用户的"理解"而非"记录"来提供真正个性化的服务时，它的记忆系统才真正成功了。
