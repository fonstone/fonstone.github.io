...
1. Coding Agent 的七个核心工具中，为什么"编辑文件"与"写文件"需要分开？两者在上下文消耗和容错性上有什么差异？
2. OpenClaw 采用 Sessionless 设计，跨消息的进程状态如何保持？文件系统状态与进程状态分别如何处理？
3. Simon Willison 的"致命三要素"是什么？为什么说"持久记忆"是攻击的放大器而非第四个必要条件？
4. 语义解析相比关键字黑名单在命令安全校验上的优势是什么？请举例说明黑名单可以被绕过的场景。
5. 试分析 Proposer-Reviewer 范式如何解决 Coding Agent 的"过早终止"问题？如果去掉 Reviewer 会产生什么后果？