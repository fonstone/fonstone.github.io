---
title: "超越物理内存的地址空间"
description: "到目前为止，在面向批处理系统的操作系统中，正在运行的任务只有一个，可以访问计算机的整个物理内存空间。如果物理内存空间不够，任务需要比较小心翼翼的地申请和释放内存，来确保在当前时间段中内存够用。在面向多道程序的协作式操作系..."
date: "2026-07-12"
order: 85
tags: ["虚拟内存", "缺页异常", "页面置换", "swap", "虚存"]
est_time: "30分钟"
---
## 本节导读
...
![overlay-tech.png](/images/rust-os/rcore/chapter4/overlay-tech.png)
...
![swap-tech.png](/images/rust-os/rcore/chapter4/swap-tech.png)
...
![mem-levels.png](/images/rust-os/rcore/chapter4/mem-levels.png)
...
$$
AvgTime = ( T\_m \* P\_{hit} + T\_s \* P\_{miss} ) / ( P\_{hit} + P\_{miss} )
$$

这里 $T\_m$ 是访问内存单元的时间，$T\_s$ 是访问存储设备的时间， $P\_{hit}$ 是访存命中次数， $P\_{miss}$ 是访存未命中次数。 访存命中率是：

$$
HitRatio = P\_{hit} / ( P\_{hit} + P\_{miss} )
$$
...
![fifo-strategy.png](/images/rust-os/rcore/chapter4/fifo-strategy.png)
...
![lru-strategy.png](/images/rust-os/rcore/chapter4/lru-strategy.png)
...
根据之前的 [页面置换机制](#section-11) 中处理器访问数据的过程，我们可以了解到在访问数据前，想要查找位于TLB或内存中的页表项，所以我们可以对页表项中的某些位进行一定的扩展，用来表示应用访问内存的情况。如果要表示应用访问某页的情况，可以用一位就够了，“1”表示近期访问了某页，“0”表示近期没有访问某页。在最小硬件成本的前提下，我们就可在页表项中的一个bit（use bit 使用位, 也称 reference bit 引用位）来表示页表项对应的虚拟页的访问情况。
...
![clock-strategy.png](/images/rust-os/rcore/chapter4/clock-strategy.png)
...
![workingset-strategy.png](/images/rust-os/rcore/chapter4/workingset-strategy.png)
...
![page-fault-rate-strategy.png](/images/rust-os/rcore/chapter4/page-fault-rate-strategy.png)
