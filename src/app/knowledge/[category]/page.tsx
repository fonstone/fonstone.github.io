      ...
      <aside className="hidden lg:block w-56 shrink-0">
        <div className="sticky top-8">
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/60 dark:bg-slate-900/40 backdrop-blur-sm p-4">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">分类信息</h3>
            <div className="flex flex-col gap-3 text-xs text-slate-500 dark:text-slate-400">
              <div>
                <span className="block text-slate-400 dark:text-slate-500 mb-1">文章数</span>
                <span className="font-medium text-slate-700 dark:text-slate-300">{current.posts.length} 篇</span>
              </div>
              <div>
                <span className="block text-slate-400 dark:text-slate-500 mb-1">标签</span>
                <div className="flex flex-wrap gap-1">
                  {Array.from(new Set(current.posts.flatMap((p) => p.tags))).slice(0, 8).map((tag) => (
                    <span key={tag} className="rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-slate-500 dark:text-slate-400">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div>
                <span className="block text-slate-400 dark:text-slate-500 mb-1">最新文章</span>
                <span className="font-medium text-slate-700 dark:text-slate-300 line-clamp-2">
                  {current.posts[0]?.title || "无"}
                </span>
              </div>
            </div>
          </div>
