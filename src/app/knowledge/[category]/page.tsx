...
export default async function KnowledgeCategoryPage({
  ...
  return (
    <article className="flex-1 min-w-0">
      <header className="flex flex-col gap-2 mb-8 pb-6 border-b border-gray-200">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-gray-900">{category}</h1>
      ...
      <div className="flex flex-col divide-y divide-gray-200">
        {current.posts.map((post) => (
          <Link
            ...
            className="group flex flex-col gap-2 py-5 transition-colors hover:bg-gray-50 -mx-2 px-2 rounded-lg"
          >
            <div className="flex items-baseline justify-between gap-4">
              ...
              {post.date && (
                <time className="shrink-0 text-sm text-gray-400 tabular-nums">
                  {post.date}
                </time>
              )}
            </div>
